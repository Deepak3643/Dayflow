...
document.getElementById('add-routine').addEventListener('click', () => {
  ...
  document.querySelector('#routine-dialog h2').textContent = 'Add a routine/task';
...
document.addEventListener('click', event => {
  if (!event.target.closest('.routine-actions')) {
    document.querySelectorAll('.routine-actions.open').forEach(item => {
...
document.getElementById('set-focus').addEventListener('click', () => {
  ...
  if (focus) {
    document.querySelector('.focus-card p:last-child').textContent = focus;
    toast('Today’s focus saved.');
  }
});
...
profileMenu.addEventListener('click', event => {
  ...
  if (action === 'profile') {
    ...
    toast('Settings are ready to customize next.');
  } else {
...
document.getElementById('notification-button').addEventListener('click', () => {
  toast('You are all caught up — great work!');
});

accountDialog.addEventListener('close', () => {
  if (accountDialog.returnValue !== 'default') {
    return;
  }
  ...
  if (photo) {
    ...
    reader.addEventListener('load', () => {
      ...
      setAvatar(name);
      toast('Profile and picture saved.');
    });
...
function normaliseHabits() {
  ...
  const data = savedHabits[name];
    ...
    if (data.lastCheckIn === todayKey) {
      data.checked = true;
    } else if (data.lastCheckIn === yesterdayKey) {
      data.checked = false;
    } else {
      data.streak = 0;
      data.checked = false;
...
document.querySelectorAll('.habit').forEach(habit => {
  ...
  habit.addEventListener('click', () => {
    ...
    if (current.checked) {
      return toast(`${name} is already checked in today.`);
    }

    current.checked = true;
    current.streak++;
    current.lastCheckIn = todayKey;
...
document.getElementById('reset-habits').addEventListener('click', () => {
  Object.keys(savedHabits).forEach(name => {
    savedHabits[name].checked = false;
    savedHabits[name].streak = 0;
    savedHabits[name].lastCheckIn = null;
  });
  localStorage.setItem(habitKey, JSON.stringify(savedHabits));
...
const activity = [
  { steps: 5240, minutes: 29, kcal: 1870 },
  { steps: 6980, minutes: 37, kcal: 2015 },
  { steps: 6105, minutes: 31, kcal: 1940 },
  { steps: 7842, minutes: 42, kcal: 2180 },
  { steps: 5820, minutes: 26, kcal: 1905 },
  { steps: 7210, minutes: 39, kcal: 2090 },
  { steps: 4380, minutes: 21, kcal: 1760 }
];
...
bars.forEach((bar, index) => {
  bar.addEventListener('click', () => selectActivity(index));
});

days.forEach((day, index) => {
  day.addEventListener('click', () => selectActivity(index));
});
...
document.querySelector('.more').addEventListener('click', () => {
  ...
  if (!steps) {
    return toast('Please enter a valid step count.');
  }

  activity[3] = {
    steps,
    minutes: Math.round(steps / 185),
    kcal: Math.round(1600 + steps / 13)
  };
